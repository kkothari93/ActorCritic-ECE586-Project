...
class PolicyEstimator:
    ...
    def _build_train_op(self):
        ...
        self.loss = -tf.log(
            ...
            self.advantage_train - self.lamb * self.norm_dist.entropy() # offset increases stability and score too!
...
if __name__ == "__main__":
    policy_lr, value_lr, lamb, gamma = [0.00046415888336127773, 0.021544346900318822, 0.035938136638046257, 0.98999999999999999] #
    # policy_lr, value_lr, lamb, gamma = [0.00046415888336127773, 0.021544346900318822, 0.035938136638046257, 0.9] #
    # loss = actor_critic(episodes=1000, gamma=gamma, display=False, lamb=lamb, policy_lr=policy_lr, value_lr=value_lr)
    phi = state_lfa(env)
    loss = ActorCritic(env, phi, exp_buffer_length=100,episodes=1000, gamma=gamma, display=False, lamb=lamb, policy_lr=policy_lr, value_lr=value_lr).run()
    print(-loss)
    env.close()
